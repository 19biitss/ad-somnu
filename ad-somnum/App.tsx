...
import { createHipnosChat, generateTenguPlan } from './services/geminiService';
import { 
...
// --- Sub-components defined here for simplicity in single-file logic, usually separated ---
...
const BreathingView = ({ t }: { t: any }) => {
  ...
  useEffect(() => {
    ...
    const runCycle = () => {
      ...
      // 3. EXHALE (8 seconds) -> Shrink
      const exhaleTimeout = setTimeout(() => {
        setPhaseText(t.exhale);
        setScale(1);
        setDuration(8000); 
      }, 11000); // 4000 (inhale) + 7000 (hold) = 11000
    ...
    // Loop every 19 seconds (4 inhale + 7 hold + 8 exhale)
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-4 text-center">
      ...
      <div className="relative w-64 h-64 flex items-center justify-center">
        ...
        {/* Breathing Circle */}
        {/* Dark Mode: Cyan to Indigo. Light Mode: Soft Indigo to Purple */}
        <div 
...
const MusicView = ({ t }: { t: any }) => {
  ...
  useEffect(() => {
    ...
    if (playingId !== null) {
      ...
      if (track && track.url) {
        ...
        audioRef.current.loop = true; // Loop the soothing sound
        audioRef.current.play().catch(e => {
            console.error("Audio playback failed:", e);
            setPlayingId(null); // Reset UI if play fails
        });
      }
    }
  ...
  // Stop audio when component unmounts (leaving the tab)
  useEffect(() => {
    return () => {
      if (audioRef.current) {
        audioRef.current.pause();
  ...
  return (
    <div className="flex flex-col items-center h-full overflow-y-auto px-4 pt-4 pb-24 scrollbar-hide">
      ...
      <RenderSection 
        ...
        tracks={MUSIC_DATA.filter(m => m.category === 'nature')} 
      />
      <RenderSection 
        title={t.categoryMelody} 
        tracks={MUSIC_DATA.filter(m => m.category === 'melody')} 
      />
    </div>
  );
};

// 4. Hipnos Chat Interface (Preserved)
...
const ChatView = ({ t, name, intro, language }: ChatViewProps) => {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      setMessages([{ id: 'err', role: 'model', text: 'API Key missing.', timestamp: Date.now() }]);
      return;
    }
  ...
  const handleSend = async () => {
    ...
    if (!chatSession) {
        // Retry init if came back online? simplified for now just generic error
        const errorMsg: ChatMessage = { id: 'err', role: 'model', text: t.errorGeneric, timestamp: Date.now() };
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-md mx-auto">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-2 pb-4 scrollbar-hide space-y-4">
          ...
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm ${
              msg.role === 'user' 
                ...
                : 'bg-white dark:bg-white/10 text-slate-700 dark:text-slate-200 rounded-tl-none border border-slate-100 dark:border-white/5'
            }`}>
              {msg.text}
            </div>
          </div>
        ...
        {loading && (
          <div className="flex justify-start">
             <div className="bg-white dark:bg-white/10 p-3 rounded-2xl rounded-tl-none flex space-x-2 items-center border border-slate-100 dark:border-white/5 shadow-sm">
      ...
      <div className="p-4 bg-transparent border-t border-slate-200/50 dark:border-white/5">
        <div className="flex items-center bg-white/50 dark:bg-slate-800/50 rounded-full px-4 py-2 border border-slate-300/50 dark:border-white/10 shadow-sm backdrop-blur-sm transition-all focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400/20">
          <input 
            type="text" 
            value={input}
            ...
            className="flex-1 bg-transparent border-none outline-none text-slate-800 dark:text-white text-sm placeholder-slate-500 dark:placeholder-slate-400"
          />
          <button onClick={handleSend} className="ml-2 text-indigo-500 hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-300">
...
// 5. New Tengu Plan View (Form based)
const TenguView = ({ t, language }: { t: any; language: Language }) => {
  ...
  const handleSubmit = async () => {
    if (!dinnerTime || !bedTime) return; // Simple validation
    
    // Offline Check
    if (!navigator.onLine) {
        setPlan(t.errorNoInternet);
        return;
    }
  ...
  if (plan) {
    return (
      <div className="flex flex-col h-full w-full max-w-md mx-auto p-6 overflow-y-auto pb-24 scrollbar-hide">
         ...
         <div className="bg-white/60 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-2xl shadow-sm text-sm leading-7 text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-light">
           {plan}
         </div>

         <button 
            onClick={handleReset}
            className="mt-8 w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-medium tracking-wide shadow-lg shadow-indigo-500/20 transition-all transform hover:scale-[1.02]"
          >
            {t.tenguBtnReset}
          </button>
      </div>
      ...
      <div className="space-y-5">
        <div>
          ...
          <input 
            ...
            className="w-full p-4 rounded-xl bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 text-day-text dark:text-white outline-none focus:border-indigo-500 transition-colors"
          />
        </div>
        ...
        <div>
          ...
          <textarea 
            ...
            rows={3}
            className="w-full p-4 rounded-xl bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 text-day-text dark:text-white outline-none focus:border-indigo-500 transition-colors text-sm resize-none"
          />
        </div>

        <button 
          onClick={handleSubmit}
          disabled={loading || !dinnerTime || !bedTime}
          className={`w-full py-4 rounded-xl font-medium tracking-wide shadow-lg transition-all transform ${
            loading || !dinnerTime || !bedTime
...
const SettingsView = ({ 
  t, language, setLanguage, theme, setTheme 
}: { 
  t: any, 
  language: Language, 
  ...
  return (
    <div className="p-6 w-full max-w-md mx-auto space-y-8">
      <div>
        <h2 className="text-2xl font-light text-day-text dark:text-night-text tracking-wide mb-1">{t.navSettings}</h2>
        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-6">{t.descSettings}</p>
      </div>

      {/* Language */}
      <div className="space-y-2">
        <label className="text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-500">{t.settingsLang}</label>
        <div className="relative">
          <select 
            value={language}
            onChange={(e) => setLanguage(e.target.value as Language)}
            className="w-full p-3 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-day-text dark:text-night-text outline-none focus:border-indigo-500 dark:focus:border-indigo-400 appearance-none transition-colors shadow-sm dark:shadow-none"
          >
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Language.ES}>Español</option>
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Language.CA}>Català</option>
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Language.EN}>English</option>
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Language.FR}>Français</option>
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Language.DE}>Deutsch</option>
          </select>
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none text-slate-400">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4"><path d="m6 9 6 6 6-6"/></svg>
          </div>
        </div>
      </div>
      ...
      {/* Theme */}
      <div className="space-y-2">
        <label className="text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-500">{t.settingsTheme}</label>
        <div className="relative">
          <select 
            value={theme}
            ...
            className="w-full p-3 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-day-text dark:text-night-text outline-none focus:border-indigo-500 dark:focus:border-indigo-400 appearance-none transition-colors shadow-sm dark:shadow-none"
          >
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Theme.DARK}>{t.themeDark}</option>
            <option className="text-black bg-white dark:bg-slate-800 dark:text-white" value={Theme.LIGHT}>{t.themeLight}</option>
          </select>
           <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none text-slate-400">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4"><path d="m6 9 6 6 6-6"/></svg>
          </div>
      ...
      <div className="space-y-2 pt-6 border-t border-slate-200 dark:border-slate-700/50">
        <label className="text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-500">{t.settingsInfo}</label>
        <p className="text-sm text-slate-600 dark:text-slate-400">{t.infoPlaceholder}</p>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  // Apply theme class to body/wrapper
  useEffect(() => {
  ...
  if (showSplash) {
    return <SplashScreen onFinish={() => setShowSplash(false)} translations={t} />;
  }

  // Updated Background Logic
  const bgClass = theme === Theme.DARK 
    ? "bg-gradient-to-b from-night-800 to-night-900 text-night-text" 
    : "bg-day-100 text-day-text";

  return (
    <div className={`h-screen w-full flex flex-col overflow-hidden transition-colors duration-500 ${bgClass}`}>
      {/* Main Content Area */}
      ...
      {/* Bottom Navigation */}
      <nav className={`h-20 flex-shrink-0 flex justify-around items-center px-2 pb-2 backdrop-blur-lg border-t ${theme === Theme.DARK ? 'bg-night-900/90 border-white/5' : 'bg-white/90 border-slate-200'}`}>
        
        <NavButton 
          active={view === View.BREATHING} 
          onClick={() => setView(View.BREATHING)} 
          icon={<WindIcon className="w-6 h-6" />} 
          label={t.navBreathing} 
          theme={theme}
        />
        <NavButton 
          active={view === View.MUSIC} 
        ...
        />
        <NavButton 
          active={view === View.HIPNOS} 
          onClick={() => setView(View.HIPNOS)} 
          icon={<MessageSquareIcon className="w-6 h-6" />} 
          label={t.navHipnos} 
          theme={theme}
        />
        <NavButton 
          active={view === View.TENGU} 
          onClick={() => setView(View.TENGU)} 
...
const NavButton = ({ active, onClick, icon, label, theme }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string, theme: Theme }) => {
  // Updated Active/Inactive Colors to avoid Magenta/Pink
  const activeClass = theme === Theme.DARK ? 'text-indigo-400' : 'text-indigo-600';
  const inactiveClass = theme === Theme.DARK ? 'text-slate-500 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600';

  return (
    <button 
      onClick={onClick} 
      className={`flex flex-col items-center justify-center w-full h-full transition-colors duration-300 ${active ? activeClass : inactiveClass}`}
    >
      ...
      {active && <div className={`w-1 h-1 rounded-full mt-1 ${theme === Theme.DARK ? 'bg-indigo-400' : 'bg-indigo-600'}`} />}
    </button>
  );
};